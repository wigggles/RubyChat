...
class GUI::GraphBar < GUI::Component
  ...
  # Create klass object.
  #---------------------------------------------------------------------------------------------------------
  def initialize(init_data: {}, label: nil, **supers)
    super(supers)
    # The draw size is rectangular, so width and height both share.
    @disposed = false
    @graph_data = init_data # Used to plot data to a graph.
  ...
  #---------------------------------------------------------------------------------------------------------
  # Puts a numeric_value into the graphing data.
  #---------------------------------------------------------------------------------------------------------
  ...
  #---------------------------------------------------------------------------------------------------------
  # Since drawing text takes time, so text used in the key display for a graph is rendered into a cache.
  ...
  def render_graph_key
    image ||= Gosu.render(@drawsize, @drawsize, retro: true) do
      sy = 0
      Gosu.rotate(90, @drawsize / 2, @drawsize / 2) do
        @graph_data.each do |key, _value|
