...
class TCPsession
  ...
  def is_self?(this_user_id = nil)
    ...
    @client_self.ref_id == this_user_id
  end
  ...
  def request_sync_client
    Logger.info('TCPsession',
                "Local client is requesting a change to its public description. (#{@client_self.inspect})",
                tags: %i[Network Package Client])
  ...
  def empty_data_package
    ...
    Package.new(nil, @client_self.ref_id)
  end
  ...
  def send_msg(sending_data, pack_data: true)
    ...
    when String
      ...
      Logger.error('TCPsession',
                   "Can only send String messages through the socket. not: #{sending_data.class}",
                   tags: [:Network])
      return nil
    end
    # return success
  ...
  def socket_puts(string)
    ...
    begin
      if TCPsession::USE_RAW_STRING_PACKAGE
        ...
        # Convert the string of characters into an array, then pack that array into a string for a hex
        # representation of those byte characters. Doing this prevents the chances of sending bytes used
        # internally for networks. The down side of doing this is that the message size is halved as it
        # takes two characters to show a hex value for a byte.
        @socket.puts(string.bytes.pack('c*').unpack1('H*'))
      end
    # catch errors and if known to not be critical, handle in a safe way
      ...
      when Errno::ENOTCONN
        ...
        Logger.error('TCPsession', "Write: #{e.inspect}",
                     tags: [:Network])
      end
      return false
    end
  ...
  #---------------------------------------------------------------------------------------------------------
  # Is blocking function, wait for a connected network session message. All incoming data flows here first.
  def await_data_msg(use_package = true)
    ...
    begin
      ...
      # this requires an extra step in packaging the data if enabled and also doubles the sending
      # string's size.
      response_string = [response_string].pack('H*') unless TCPsession::USE_RAW_STRING_PACKAGE
    rescue StandardError => e
      case e
      ...
      when Errno::EPIPE
        ...
        Logger.error('TCPsession', "Read: #{e.inspect}",
                     tags: [:Network])
      end
    end
