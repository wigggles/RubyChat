...
class MainState
  ...
  def initialize
    ...
    @command_field = GUI::TextField.new(
      ...
      pos_x: @console_box.x,
      pos_y: @console_box.y + @console_box.height + 4,
      action: :text_action,
    ...
    @buttons = [
      GUI::Button.new(
        ...
        pos_x: $application.width - 4, pos_y: 4, align: :right
      ),
      GUI::CheckBox.new(
        owner: self, action: :checkbox_action, radius: 12,
        pos_x: $application.width - 148, pos_y: 28, align: :center
      )
    ]
    # set the default game world
    options = {
      pos_x: @console_box.right + 4,
      pos_y: @console_box.y,
  ...
  def process_incoming_session_dataPackage(package)
    ...
    own_package = true
    own_package = $application.current_session.is_self?(package.ref_id) unless package.ref_id.nil?
    status_string = ''
    ...
    when TCPsession::Package::DATAMODE::STRING
      if own_package
        ...
        status_string = if client_description
                          "(#{client_description.username})> #{package.data}"
                        else
                          "(#{package.ref_id})> #{package.data}"
                        end
      end
  ...
  def draw_local_description
    username = if $application.self_client_description.nil?
                 "'nil'"
               else
                 $application.self_client_description.username
               end
