...
module Configuration
  ...
  def self.generate_new_ref_id(as_string: true, micro: false, clamp: false, packed: false)
    ...
    new_id = if clamp
               ...
               ((Time.now.to_f * 10_000_000).round % 42_949_672_950) # actually *4_294_967_295
             else
               ...
               (Time.now.to_f * 10_000_000).round
    ...
    if as_string
      ...
      new_id = if micro # only grab 3 bytes (6 characters), quite high chances of generating an existing id
                 # 'micro' can support 16_777_215 unique IDs, a thousandth used in one instance is 16_777 IDs.
                 # Chances roll over every
                 new_id.slice(6...8).concat(new_id.slice(10...new_id.size))
