...
#
#   You can also run individual tests by providing their id and an optional times argument. if no --times, does 10,000
...
#     ruby benchmark.rb
#
# * Note, this may seem excessive, but this organizes the testing in a manor that makes batches configurable and allow
#     for specific targets however those may change during the usage of fetching commonly used bench tests.
#
#=====================================================================================================================
...
require './benches/registry'      # Magic number container for defining tests.
require './benches/test_groups'   # Each test group, for running whole tests.
require './benches/branches'      # Where the tests are outlined.

class BenchTests
  #---------------------------------------------------------------------------------------------------------
  # Generally blank but an availabel spot for benchmarking code snippets quickly.
  #---------------------------------------------------------------------------------------------------------
  ...
  #---------------------------------------------------------------------------------------------------------
  # Run a selected bench test suite. The names of these groups are found in the registry of *Magic Numbers*
  #---------------------------------------------------------------------------------------------------------
  def run(group)
    case group
  ...
  #---------------------------------------------------------------------------------------------------------
  # Changes the seed each time its called to stay random.
  #---------------------------------------------------------------------------------------------------------
  def ranged_rand(seed, max)
    # https://ruby-doc.org/core-2.2.0/Random.html
    extra_step = Random.new(max + seed)
    extra_step.rand(max) + rand(max) % max
    # string = Random.new.bytes(10) # random string
  end

  #---------------------------------------------------------------------------------------------------------
  # Draw column definition header over string information table display.
  #---------------------------------------------------------------------------------------------------------
  ...
  #---------------------------------------------------------------------------------------------------------
  # All text writes are funneled through here. This can be set up to clone printed text and send it to GUI
  # or any Logger, also displays the information directly into the console or can write to a file.
  #---------------------------------------------------------------------------------------------------------
...
class StressAux_Class
  ...
  def arguments_strict(one, two, three, four, five, six, seven, eight, nine, ten); end

  def arguments_array(*options); end

  def arguments_symbol(one:, two:, three:, four:, five:, six:, seven:, eight:, nine:, ten:); end

  def arguments_hash(**options); end

  def arguments_value(options = {}); end
end
...
module StressAux_Module
  ...
  def self.arguments_strict(one, two, three, four, five, six, seven, eight, nine, ten); end

  def self.arguments_array(*options); end

  def self.arguments_symbol(one:, two:, three:, four:, five:, six:, seven:, eight:, nine:, ten:); end

  def self.arguments_hash(**options); end

  def self.arguments_value(options = {}); end
end
...
if RUN_TEST
  ...
  elsif RUN_TEST.is_a?(Integer) # run one specific test
